...
func TestCalculateCrcValidate(t *testing.T) {
	data := []byte{0xa0, 0x08, 0x02, 0x23, 0xc9, 0x93}
	if calculateHdlcCRC(data) != 0x43e4 {
		t.Error("CRC mismatch for known SNRM data")
	}
}

func TestCalculateCrcInvalidate(t *testing.T) {
	data := []byte{0xa0, 0x08, 0x02, 0x23, 0xc9, 0x93}
	if calculateHdlcCRC(data) == 0x43e5 {
		t.Error("CRC unexpectedly matched wrong value")
	}
}
